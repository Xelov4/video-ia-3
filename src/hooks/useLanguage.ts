...
export function useLocalizedRouting() {
  ...
  const navigateToLocalized = useCallback(
    (
      ...
      const localizedPath =
        targetLanguage === defaultLocale ? cleanPath : `/${targetLanguage}${cleanPath}`;
  ...
  const getLocalizedPath = useCallback(
    (path: string, targetLanguage: SupportedLocale = currentLanguage) => {
      const cleanPath = path.startsWith('/') ? path : `/${path}`;
  ...
  const changeLanguage = useCallback(
    (targetLanguage: SupportedLocale) => {
      ...
      const currentPrefix =
        currentLanguage === defaultLocale ? '' : `/${currentLanguage}`;
...
export function useAlternateUrls() {
  ...
  const alternateUrls = useMemo(() => {
    ...
    return supportedLocales.reduce(
      (acc, lang) => {
        const localizedPath =
          lang === defaultLocale ? pathWithoutLang : `/${lang}${pathWithoutLang}`;
...
export function useShortUrlConstruction() {
  ...
  const getCategoryUrl = useCallback(
    (slug: string, lang?: SupportedLocale) => {
   ...
   */
  const getUseCaseUrl = useCallback(
    (slug: string, lang?: SupportedLocale) => {
      const targetLang = lang || currentLanguage;
