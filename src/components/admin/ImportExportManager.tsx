...
export const ImportExportManager = () => {
  ...
  const importJobs: ImportJob[] = [
    ...
    },
    {
      ...
      errors: ['Invalid URL format on row 15', 'Missing required field "toolName" on row 23']
    }
  ]
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {importJobs.filter(job => job.status === 'running').map((job) => (
                  ...
                  <div key={job.id} className="flex items-center justify-between p-3 border rounded-lg">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-muted-foreground">
                          {job.recordsProcessed} / {job.totalRecords} enregistrements • {job.startTime}
                        </div>
                      </div>
                      <Badge variant="outline">{job.type}</Badge>
                    </div>
