...
export function AdminToolsContent({ searchParams }: AdminToolsContentProps) {
  ...
  const [filters, setFilters] = useState<AdminFilterState>(() => ({
    ...
    searchFields: searchParams.searchFields
      ? searchParams.searchFields.split(',')
      : ['all'],
  ...
  const performAdvancedSearch = useCallback(
    (tools: Tool[], filters: AdminFilterState) => {
      let filtered = [...tools];
      ...
      if (filters.search) {
        ...
        filtered = filtered.filter(tool => {
          ...
          if (
            filters.searchFields.includes('all') ||
          ...
          if (filters.searchFields.includes('description')) {
            ...
            if (tool.tool_description) {
              matches =
                matches || tool.tool_description.toLowerCase().includes(searchTerm);
            }
          }
  ...
  const loadToolsWithFilters = useCallback(
    async (newFilters: AdminFilterState) => {
      try {
        setLoading(true);
        setError(null);
  ...
  const generateMockTools = (): Tool[] => {
    ...
    for (let i = 0; i < 100; i++) {
      mockTools.push({
        id: i + 1,
        tool_name:
          toolNames[i % toolNames.length] +
          (i >= toolNames.length ? ` ${Math.floor(i / toolNames.length) + 1}` : ''),
