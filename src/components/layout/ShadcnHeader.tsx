...
export default function ShadcnHeader({ 
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault()
    if (searchQuery.trim()) {
      const searchPath = currentLanguage === 'en' 
  ...
  return (
    <header className={cn(
      ...
      'bg-slate-900 border-b border-slate-800 shadow-lg'
    )}>
      {/* Top bar with stats */}
            ...
            <form onSubmit={handleSearch} className="hidden md:block">
              <div className="relative">
                ...
                <Input
                  ...
                  className="w-64 pl-9 bg-slate-800 border-slate-700 text-white placeholder:text-slate-400 focus:bg-slate-700 focus:border-blue-500"
                />
              </div>
            </form>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button 
                  variant="outline"
                  size="sm" 
                  className="space-x-2 min-w-[120px] justify-start bg-slate-800 border-slate-700 text-white hover:bg-slate-700 hover:text-white"
                >
              ...
              <DropdownMenuContent align="end" className="w-56 p-2 bg-slate-800 border-slate-700">
                <div className="text-xs font-medium text-slate-400 px-2 py-1 mb-1">
                  {currentLanguage === 'en' ? 'Choose language' : 
                   currentLanguage === 'fr' ? 'Choisir la langue' :
                   currentLanguage === 'es' ? 'Elegir idioma' :
                   currentLanguage === 'de' ? 'Sprache wählen' :
                   currentLanguage === 'it' ? 'Scegli lingua' :
                   currentLanguage === 'nl' ? 'Kies taal' :
                   currentLanguage === 'pt' ? 'Escolher idioma' : 'Choose language'}
                </div>
                ...
                {languages.map((lang) => (
                  <DropdownMenuItem key={lang.code} asChild>
                    <Link
                      ...
                      className={cn(
                        'flex items-center space-x-3 cursor-pointer w-full rounded-md px-2 py-2 transition-colors',
                        lang.code === currentLanguage && 'bg-blue-600 text-white',
                        lang.code !== currentLanguage && 'hover:bg-slate-700 text-slate-200'
                      )}
                    >
                      ...
                      {lang.code === currentLanguage && (
                        <Badge variant="secondary" className="text-xs bg-blue-500 text-white">Active</Badge>
                      )}
                    </Link>
                  </DropdownMenuItem>
                ))}
