...
export class OpenGraphManager {
  ...
  generateToolOG(config: {
    ...
    return {
      ...
      description:
        toolDescription ||
        `Découvrez ${toolName}, un outil IA innovant pour ${toolCategory}`,
      type: 'product',
  ...
  generateCategoryOG(config: {
    ...
    const description =
      categoryDescription || `Découvrez ${toolsCount} outils IA pour ${categoryName}`;
   ...
   */
  private generateCategoryImageUrl(
    categoryName: string,
    language: SupportedLocale
  ): string {
  ...
  validateOpenGraph(config: OpenGraphConfig): {
    ...
    if (config.title.length > 60)
      warnings.push('Title should be 60 characters or less for optimal display');
    if (config.description.length > 160)
      warnings.push('Description should be 160 characters or less');
    ...
    if (config.image) {
      if (config.image.width < 1200)
        warnings.push('Image width should be at least 1200px for best quality');
      if (config.image.height < 630)
        warnings.push('Image height should be at least 630px for best quality');
      if (!config.image.alt)
        warnings.push('Image alt text is recommended for accessibility');
    }
...
export function useOpenGraph() {
  return {
    ...
    generateCategoryOG: (
      config: Parameters<typeof openGraphManager.generateCategoryOG>[0]
    ) => openGraphManager.generateCategoryOG(config),

    generateSocialMetadata: (
      config: Parameters<typeof openGraphManager.generateSocialMetadata>[0]
