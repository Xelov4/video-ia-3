...
export interface SoftwareApplicationSchema extends BaseSchema {
  ...
  url: string
  image?: string
  applicationCategory: 'BusinessApplication' | 'MultimediaApplication' | 'DesignApplication' | 'DeveloperApplication'
...
export class SchemaGenerator {
   ...
   */
  generateWebSiteSchema(language: SupportedLocale, availableLanguages: SupportedLocale[]): WebSiteSchema {
  ...
  generateToolSchema(config: {
    ...
    const categoryMapping: Record<string, SoftwareApplicationSchema['applicationCategory']> = {
    ...
    return {
      ...
      '@type': 'SoftwareApplication',
      name: tool.toolName,
      description: tool.overview || tool.toolDescription || `${tool.toolName} - AI Tool`,
      url: `${this.baseUrl}${basePath}`,
      image: tool.imageUrl || `${this.baseUrl}/images/tools/${tool.toolName.toLowerCase().replace(/\s+/g, '-')}.jpg`,
  ...
  generateCollectionPageSchema(config: {
    ...
    tools: Array<{ id: number; toolName: string; overview: string; toolCategory: string; createdAt: Date; updatedAt: Date }>
    language: SupportedLocale
    basePath: string
    breadcrumbs: Array<{ name: string; url: string }>
  }): CollectionPageSchema {
    const { categoryName, categoryDescription, tools, language, basePath, breadcrumbs } = config
  ...
  validateSchema(schema: BaseSchema): { isValid: boolean; errors: string[] } {
    ...
    if (schema['@type'] === 'SoftwareApplication') {
      ...
      if (!appSchema.description) errors.push('SoftwareApplication missing description')
...
export function useSchema() {
  return {
    ...
    generateWebSiteSchema: (language: SupportedLocale, availableLanguages: SupportedLocale[]) =>
      schemaGenerator.generateWebSiteSchema(language, availableLanguages),
    
    generateToolSchema: (config: Parameters<typeof schemaGenerator.generateToolSchema>[0]) =>
      schemaGenerator.generateToolSchema(config),
    
    generateBreadcrumbSchema: (breadcrumbs: Parameters<typeof schemaGenerator.generateBreadcrumbSchema>[0]) =>
      schemaGenerator.generateBreadcrumbSchema(breadcrumbs),
    
    generateCollectionPageSchema: (config: Parameters<typeof schemaGenerator.generateCollectionPageSchema>[0]) =>
      schemaGenerator.generateCollectionPageSchema(config),
    
    combineSchemas: (schemas: BaseSchema[]) =>
      schemaGenerator.combineSchemas(schemas),
    
    validateSchema: (schema: BaseSchema) =>
