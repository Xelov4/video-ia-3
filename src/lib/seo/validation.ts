...
export class SeoValidator {
  ...
  async validatePage(config: {
    ...
    try {
      ...
      // Validation hreflang
      if (hreflangLinks) {
        const hreflangValidation = await this.validateHreflangImplementation(
          hreflangLinks,
          url
        );
        if (!hreflangValidation.isValid) {
          errors.push(
            ...hreflangValidation.errors.map(error => ({
  ...
  async validateHreflangImplementation(
    ...
    for (const link of hreflangLinks) {
      ...
      try {
        ...
        const crossValidationResult = await this.validateCrossHreflang(
          link.href,
          currentUrl
        );
        ...
        if (!crossValidationResult) {
          warnings.push(
            `Page ${link.href} ne référence pas correctement ${currentUrl}`
          );
        }
      } catch (error) {
    ...
    if (warnings.length > 0) {
      recommendations.push(
        'Vérifier que toutes les pages référencées implémentent correctement hreflang'
      );
    }

    if (hreflangLinks.length < 3) {
      recommendations.push(
        'Considérer ajouter plus de langues pour maximiser la portée internationale'
      );
    }
  ...
  private async fetchPageData(url: string): Promise<{
    ...
    return {
      ...
      images: [{ src: '/image1.jpg', alt: 'Alt text 1' }, { src: '/image2.jpg' }],
      links: [
  ...
  private calculateMetrics(pageData: any): SeoMetrics {
    ...
    for (const [level, headings] of Object.entries(pageData.headings) as [
      string,
      string[],
    ][]) {
   ...
   */
  private async validateCrossHreflang(
    targetUrl: string,
  ...
  generateValidationReport(results: SeoValidationResult[]): {
    ...
    const summary = {
      ...
      highErrors: results.reduce(
        (sum, r) => sum + r.errors.filter(e => e.severity === 'high').length,
        0
      ),
    };
