...
export class NotFoundMonitor {
   ...
   */
  private async findSimilarPages(targetUrl: string): Promise<Array<{ url: string; similarity: number }>> {
  ...
  private calculateSimilarity(url1: string, url2: string): number {
    ...
    tokens1.forEach(token1 => {
      const bestMatch = tokens2.find(token2 => 
   ...
   */
  private detectTypoFix(url: string): { correctedUrl: string; confidence: number; typo: string } | null {
    const commonTypos = [
  ...
  private analyzePatterns() {
    ...
    this.patterns.forEach(pattern => {
      if (pattern.count >= 10 && pattern.confidence > 0.8) {
        console.log(`High-frequency pattern detected: ${pattern.pattern} (${pattern.count} occurrences)`)
  ...
  private suggestPatternRedirection(pattern: NotFoundPattern) {
    ...
    console.log(`Suggestion: Create redirect rule for pattern ${pattern.pattern} → ${pattern.suggestedFix}`)
  }
  ...
  getMonitoringStats(): MonitoringStats {
    ...
    return {
      ...
      manuallyFixed: entries.filter(e => e.status === 'resolved' && !e.autoFixed).length,
  ...
  private calculateRecentTrend(): number {
    ...
    const week1Count = entries.filter(e => 
      e.timestamp >= week1Start && e.timestamp < week2Start
    ).length
    
    const week2Count = entries.filter(e => 
      e.timestamp >= week2Start
    ).length
  ...
  private generateRecommendations(stats: MonitoringStats, patterns: NotFoundPattern[]): string[] {
    const recommendations: string[] = []
    
    if (stats.total404s > 1000) {
      recommendations.push('High volume of 404 errors detected. Consider implementing bulk redirect rules.')
    }
    
    if (stats.autoFixed / stats.total404s < 0.5) {
      recommendations.push('Low auto-fix rate. Consider adjusting ML threshold or improving suggestion algorithms.')
    }
    
    if (patterns.some(p => p.count > 50)) {
      recommendations.push('Frequent patterns detected. Create generic redirect rules for common URL patterns.')
    }
    
    const recentTrend = stats.recentTrend
    if (recentTrend > 50) {
      recommendations.push('Significant increase in 404 errors. Investigate recent deployments or content changes.')
    }
...
export function useNotFoundMonitoring() {
  return {
    ...
    getMonitoringStats: () =>
      notFoundMonitor.getMonitoringStats(),
    
    generateDetailedReport: () =>
      notFoundMonitor.generateDetailedReport()
  }
}