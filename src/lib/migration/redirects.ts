...
export class RedirectManager {
   ...
   */
  addRedirectRule(
    ruleData: Omit<RedirectRule, 'id' | 'created' | 'usageCount' | 'lastUsed'>
  ): string {
    const rule: RedirectRule = {
      ...ruleData,
      id: `custom-${Date.now()}`,
...
export const config = {
  ...
  getRedirectStats(): RedirectStats {
    ...
    const recentlyUsed = allRules.filter(
      r => r.lastUsed && Date.now() - r.lastUsed.getTime() < 24 * 60 * 60 * 1000
    ).length;
    ...
    const statusDistribution = allRules.reduce(
      (acc, rule) => {
  ...
  exportForWebServer(format: 'nginx' | 'apache' | 'cloudflare'): string {
    const allRules = [...this.rules.values(), ...this.patternRules].filter(
      r => r.active
    );
  ...
  private evaluateCondition(condition: RedirectCondition, context: any): boolean {
    ...
    const compareValue =
      !condition.caseSensitive && typeof condition.value === 'string'
...
export function useRedirects() {
  return {
    findRedirect: (
      url: string,
      context?: Parameters<typeof redirectManager.findRedirect>[1]
    ...
    getRedirectStats: () => redirectManager.getRedirectStats(),

    exportForWebServer: (
      format: Parameters<typeof redirectManager.exportForWebServer>[0]
