...
export class LanguageDetector {
  ...
  async detectLanguage(
    ...
    const results = detectionSources
      .filter(
        (result): result is PromiseFulfilledResult<DetectionResult | null> =>
   ...
   */
  private async detectFromBrowser(
    acceptLanguage?: string
  ...
  private async detectFromUserHistory(): Promise<DetectionResult | null> {
    try {
      ...
      const mostUsed = Object.entries(usage).sort(([, a], [, b]) => b - a)[0];
  ...
  private async detectFromContentAnalysis(): Promise<DetectionResult | null> {
    ...
    if (
      detectedLang.confidence > 0.6 &&
      supportedLocales.includes(detectedLang.language)
    ) {
      return {
  ...
  private computeFinalResult(results: DetectionResult[]): DetectionResult {
    ...
    const languageScores: Record<
      string,
      { score: number; sources: DetectionSource[] }
    > = {};
    ...
    const bestCandidate = Object.entries(languageScores).sort(
      ([, a], [, b]) => b.score - a.score
    )[0];
