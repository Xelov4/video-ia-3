...
class SmartStorage {
  ...
  private checkCompressionSupport(): boolean {
    try {
      return typeof CompressionStream !== 'undefined' || typeof window !== 'undefined';
    } catch {
      return false;
    }
  }
    ...
    window.addEventListener('storage', event => {
      if (event.key?.startsWith('video-ia-')) {
        ...
        window.dispatchEvent(
          new CustomEvent('video-ia-storage-sync', {
  ...
  private async compressData(data: string): Promise<string> {
    if (
      ...
      data.length < STORAGE_CONFIG.COMPRESSION_THRESHOLD
    ) {
      return data;
    }
...
export class TranslationCacheManager {
  ...
  async setCachedTranslation(
    ...
    // Cache persistent
    const cached =
      (await this.storage.getItem<Record<string, any>>(
        STORAGE_CONFIG.KEYS.TRANSLATION_CACHE,
        {}
      )) || {};
