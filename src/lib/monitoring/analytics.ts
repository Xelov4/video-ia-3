...
export class AnalyticsManager {
   ...
   */
  generateLanguageReport(language: SupportedLocale, timeRange: '24h' | '7d' | '30d' = '24h') {
    const cutoffTime = this.getCutoffTime(timeRange)
    
    const languageEvents = this.events.filter(event => 
      event.language === language && 
      event.timestamp >= cutoffTime
    )

    const languagePerformance = this.performanceMetrics.filter(metric =>
  ...
  generateGlobalDashboard() {
    const allLanguages = Array.from(new Set(this.events.map(e => e.language))) as SupportedLocale[]
...
export function useAnalytics() {
  return {
    ...
    trackLanguageEvent: (eventType: Parameters<typeof analyticsManager.trackLanguageEvent>[0], details: Record<string, any>) =>
      analyticsManager.trackLanguageEvent(eventType, details),
    
    trackBusinessEvent: (eventType: Parameters<typeof analyticsManager.trackBusinessEvent>[0], details: Record<string, any>) =>
      analyticsManager.trackBusinessEvent(eventType, details),
    
    generateLanguageReport: (language: SupportedLocale, timeRange?: Parameters<typeof analyticsManager.generateLanguageReport>[1]) =>
      analyticsManager.generateLanguageReport(language, timeRange),
    
    generateGlobalDashboard: () =>
      analyticsManager.generateGlobalDashboard()
  }
}