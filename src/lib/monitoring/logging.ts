...
export class LogAggregator {
   ...
   */
  debug(message: string, language: SupportedLocale, context?: Partial<LogContext>, metadata?: Partial<LogMetadata>): string {
    return this.log('debug', message, language, context || {}, metadata)
  }

  info(message: string, language: SupportedLocale, context?: Partial<LogContext>, metadata?: Partial<LogMetadata>): string {
    return this.log('info', message, language, context || {}, metadata)
  }

  warn(message: string, language: SupportedLocale, context?: Partial<LogContext>, metadata?: Partial<LogMetadata>): string {
    return this.log('warn', message, language, context || {}, metadata)
  }

  error(message: string, language: SupportedLocale, context?: Partial<LogContext>, metadata?: Partial<LogMetadata>): string {
    return this.log('error', message, language, context || {}, metadata)
  }

  fatal(message: string, language: SupportedLocale, context?: Partial<LogContext>, metadata?: Partial<LogMetadata>): string {
   ...
   */
  search(query: LogQuery): { entries: LogEntry[]; total: number; aggregations: LogAggregation } {
    ...
    if (query.languages && query.languages.length > 0) {
      filteredLogs = filteredLogs.filter(log => query.languages!.includes(log.language))
    }

    // Filtrer par source
    if (query.sources && query.sources.length > 0) {
      filteredLogs = filteredLogs.filter(log => query.sources!.includes(log.source.service))
    }

    // Filtrer par composant
    if (query.components && query.components.length > 0) {
      filteredLogs = filteredLogs.filter(log => query.components!.includes(log.context.component))
    }
  ...
  private triggerPatternAlert(pattern: LogPattern, entry: LogEntry) {
    console.log(`🚨 Log pattern alert: ${pattern.name} (${pattern.matchCount} matches)`)
  ...
  private matchesQuery(entry: LogEntry, query: LogQuery): boolean {
    ...
    if (query.components && !query.components.includes(entry.context.component)) return false
  ...
  private generateAggregations(logs: LogEntry[]): LogAggregation {
    ...
    const byLanguage = logs.reduce((acc, log) => {
  ...
  private calculateAverageResponseTime(logs: LogEntry[]): number {
    ...
    const totalDuration = logsWithDuration.reduce((sum, log) => sum + (log.metadata.duration || 0), 0)
  ...
  private getTopErrors(logs: LogEntry[]): LogAggregation['topErrors'] {
    ...
    const errorMessages = new Map<string, { count: number; lastSeen: Date; languages: Set<SupportedLocale> }>()

    errorLogs.forEach(log => {
      const key = log.message.substring(0, 100) // Premiers 100 chars comme clé
      if (!errorMessages.has(key)) {
        errorMessages.set(key, { count: 0, lastSeen: log.timestamp, languages: new Set() })
      }
...
export function useLogging() {
  return {
    debug: (message: string, language: SupportedLocale, context?: Partial<LogContext>, metadata?: Partial<LogMetadata>) =>
      logAggregator.debug(message, language, context, metadata),
    
    info: (message: string, language: SupportedLocale, context?: Partial<LogContext>, metadata?: Partial<LogMetadata>) =>
      logAggregator.info(message, language, context, metadata),
    
    warn: (message: string, language: SupportedLocale, context?: Partial<LogContext>, metadata?: Partial<LogMetadata>) =>
      logAggregator.warn(message, language, context, metadata),
    
    error: (message: string, language: SupportedLocale, context?: Partial<LogContext>, metadata?: Partial<LogMetadata>) =>
      logAggregator.error(message, language, context, metadata),
    
    fatal: (message: string, language: SupportedLocale, context?: Partial<LogContext>, metadata?: Partial<LogMetadata>) =>
      logAggregator.fatal(message, language, context, metadata),
    
    search: (query: LogQuery) =>
      logAggregator.search(query),
