...
export class AlertingManager {
  ...
  private initializeDefaultRules() {
    const defaultRules: Omit<AlertRule, 'id' | 'created' | 'triggerCount'>[] = [
      // Performance Alerts
      {
        ...
        severity: 'medium',
        channels: [
          { type: 'email', config: { recipients: ['dev@video-ia.net'] }, enabled: true, priority: 1 },
        ...
        escalation: {
          levels: [
            {
              delay: 30,
              channels: [{ type: 'sms', config: { number: '+1234567890' }, enabled: true, priority: 1 }],
      ...
      // Error Rate Alerts
      {
        ...
        channels: [
          ...
          { type: 'email', config: { recipients: ['tech-leads@video-ia.net'] }, enabled: true, priority: 2 }
        ],
        cooldown: 10,
        escalation: {
          levels: [
      ...
      // Core Web Vitals Alerts
      {
        ...
        severity: 'medium',
        channels: [
          { type: 'email', config: { recipients: ['frontend@video-ia.net'] }, enabled: true, priority: 1 }
        ],
        cooldown: 30,
        active: true,
        lastTriggered: undefined
      ...
      // Traffic Anomaly
      {
        ...
        severity: 'low',
        channels: [
          { type: 'slack', config: { channel: '#monitoring' }, enabled: true, priority: 1 }
        ],
        cooldown: 60,
        active: true,
        lastTriggered: undefined
      ...
      // Translation Issues
      {
        ...
        severity: 'medium',
        channels: [
          { type: 'email', config: { recipients: ['content@video-ia.net'] }, enabled: true, priority: 1 }
        ],
        cooldown: 45,
        active: true,
        lastTriggered: undefined
  ...
  private async triggerAlert(
    ...
    console.log(`🚨 Alert triggered: ${rule.name} for ${language} (${value} > ${threshold})`)
  }
...
${alert.context.suggestedActions.map(action => `• ${action}`).join('\n')}
  ...
  private checkIncidentCorrelation(alert: Alert): void {
    ...
    const recentAlerts = Array.from(this.activeAlerts.values()).filter(a => 
      a.id !== alert.id &&
      Date.now() - a.created.getTime() < 30 * 60 * 1000
    )
    ...
    if (recentAlerts.length >= 2) {
      ...
      console.log(`🔗 Incident correlation detected: ${correlationId} (${correlation.confidence}% confidence)`)
    }
  }
   ...
   */
  private checkAlertResolution(rule: AlertRule, value: number, language: SupportedLocale): void {
  ...
  getAlertingStats(): AlertingStats {
    ...
    const alertsByLanguage = activeAlerts.reduce((acc, alert) => {
    ...
    const meanTimeToResolve = resolvedAlerts.length > 0
  ...
  private checkPercentageChange(rule: AlertRule, currentValue: number, threshold: number): boolean {
    // Implementation would check historical data for percentage change
    return false // Simplified
  }

  private checkAnomalyDetection(rule: AlertRule, value: number, threshold: number, language: SupportedLocale): boolean {
    const historicalKey = `${rule.metric}-${language}`
    const historicalValues = this.thresholdLearning.get(historicalKey) || []
  ...
  private detectIncidentPattern(alerts: Alert[]): string {
    ...
    if (uniqueMetrics.includes('error_rate') && uniqueMetrics.includes('api_response_time')) {
  ...
  private calculateCorrelationConfidence(alerts: Alert[]): number {
    ...
    const timeSpread = Math.max(...alerts.map(a => a.created.getTime())) - 
  ...
  private checkResolutionCondition(rule: AlertRule, value: number, threshold: number): boolean {
  ...
  private async processEscalation(alert: Alert, rule: AlertRule): Promise<void> {
    ...
    if (shouldEscalate) {
      ...
      console.log(`⬆️ Alert escalated to level ${alert.escalationLevel}: ${alert.ruleName}`)
    }
  }
}
...
export function useAlerting() {
  return {
    evaluateMetric: (metric: string, value: number, language: SupportedLocale, context?: Partial<AlertContext>) =>
      alertingManager.evaluateMetric(metric, value, language, context),
    
    getAlertingStats: () =>
      alertingManager.getAlertingStats(),
    
    getActiveAlerts: () =>
      Array.from(alertingManager['activeAlerts'].values()),
    
    getAlertHistory: (limit = 50) =>
      alertingManager['alertHistory'].slice(-limit),
    
    getIncidentCorrelations: () =>
      Array.from(alertingManager['correlations'].values())
  }
}