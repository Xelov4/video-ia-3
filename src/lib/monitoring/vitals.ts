...
export class WebVitalsTracker {
  ...
  private recordVital(type: keyof WebVitalsMetrics, metric: VitalMetric) {
    ...
    console.log(`📊 ${type.toUpperCase()}: ${metric.value}ms (${metric.rating}) - ${this.currentLanguage}`)
  }

  /**
   * Tracker le changement de langue
   */
  trackLanguageSwitch(fromLanguage: SupportedLocale, toLanguage: SupportedLocale): void {
   ...
   */
  getLanguageAnalysis(language: SupportedLocale, period: VitalsAnalysis['period'] = 'day'): VitalsAnalysis {
  ...
  exportForDashboard(): {
    ...
    const languageDistribution = allData.reduce((acc, data) => {
    ...
    return {
      summary: {
        ...
        criticalIssues: languageBreakdown.reduce((sum, lb) => 
          sum + lb.issues.filter(i => i.severity === 'critical').length, 0
        )
      },
      languageBreakdown,
  ...
  private detectCurrentLanguage(): SupportedLocale {
    if (typeof window !== 'undefined') {
      ...
      if (langMatch && ['en', 'fr', 'es', 'it', 'de', 'nl', 'pt'].includes(langMatch[1])) {
  ...
  private getRating(type: keyof VitalsThresholds, value: number): VitalMetric['rating'] {
  ...
  private getPerformanceContext(): PerformanceContext {
    ...
    else if (url.includes('/tools') && !url.match(/\/tools\/[^\/]+$/)) pageType = 'tools'
  ...
  private measureResourceTiming(type: string): number {
    ...
    const relevantResources = resources.filter(resource => {
      ...
      if (type === 'font') return resource.name.includes('.woff') || resource.name.includes('.ttf')
    ...
    return relevantResources.reduce((sum, resource) => sum + resource.duration, 0) / relevantResources.length
  }
  ...
  private sendVitalToAnalytics(type: keyof WebVitalsMetrics, metric: VitalMetric, language: SupportedLocale) {
    // Google Analytics 4
  ...
  private checkPerformanceThresholds(type: keyof WebVitalsMetrics, metric: VitalMetric) {
  ...
  private isInPeriod(timestamp: Date, period: VitalsAnalysis['period']): boolean {
    ...
    switch (period) {
      case 'hour': return diffMs <= 60 * 60 * 1000
      case 'day': return diffMs <= 24 * 60 * 60 * 1000
  ...
  private getEmptyAnalysis(language: SupportedLocale, period: VitalsAnalysis['period']): VitalsAnalysis {
  ...
  private calculateAverageMetrics(data: LanguageVitals[]): WebVitalsMetrics {
    const metrics: (keyof WebVitalsMetrics)[] = ['lcp', 'fid', 'cls', 'fcp', 'ttfb', 'inp']
    const averages: Partial<WebVitalsMetrics> = {}

    metrics.forEach(metric => {
      const values = data.map(d => d.vitals[metric]?.value).filter(v => v !== undefined) as number[]
      if (values.length > 0) {
  ...
  private calculatePercentileMetrics(data: LanguageVitals[], percentile: number): WebVitalsMetrics {
    const metrics: (keyof WebVitalsMetrics)[] = ['lcp', 'fid', 'cls', 'fcp', 'ttfb', 'inp']
  ...
  private calculateTrends(data: LanguageVitals[], period: VitalsAnalysis['period']): VitalsAnalysis['trends'] {
  ...
  private identifyPerformanceIssues(data: LanguageVitals[]): PerformanceIssue[] {
    ...
    if (averages.cls.rating === 'poor') {
      issues.push({
        ...
        suggestedFix: 'Set explicit dimensions for images and reserve space for dynamic content',
        languages: [this.currentLanguage],
        devices: ['mobile']
      })
    }
  ...
  private calculateGlobalScore(breakdown: Array<{ score: number }>): number {
    ...
    return Math.round(breakdown.reduce((sum, b) => sum + b.score, 0) / breakdown.length)
  }
...
export function useWebVitals() {
  return {
    ...
    getLanguageAnalysis: (language: SupportedLocale, period?: VitalsAnalysis['period']) =>
      webVitalsTracker.getLanguageAnalysis(language, period),
    
    getLanguageComparison: () =>
      webVitalsTracker.getLanguageComparison(),
    
    exportForDashboard: () =>
      webVitalsTracker.exportForDashboard()
  }
}