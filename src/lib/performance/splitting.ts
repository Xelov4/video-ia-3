...
export class BundleOptimizer {
  ...
  private initializeBundleConfigs() {
    const supportedLanguages: SupportedLocale[] = [
      'en',
      'fr',
      'es',
      'it',
  ...
  generateLoadingStrategy(config: {
    ...
    if (userConnection === 'slow-2g' || userConnection === '2g') {
      ...
      strategy.lazy.push('components-secondary', 'utils-advanced', 'analytics');
      ...
      strategy.prefetch.push(...this.getPrefetchCandidates(route, language));
    }
...
export default function Component(props) {
  ...
  analyzeBundlePerformance(): {
    ...
    if (totalSize > this.performanceBudget.maxTotalSize * 7) {
      // 7 langues
...
export function useBundleOptimization() {
  return {
    generateLoadingStrategy: (
      config: Parameters<typeof bundleOptimizer.generateLoadingStrategy>[0]
    ) => bundleOptimizer.generateLoadingStrategy(config),

    analyzeBundlePerformance: () => bundleOptimizer.analyzeBundlePerformance(),

    createLazyComponent: (
      componentPath: string,
      options?: Parameters<typeof bundleOptimizer.createLazyComponent>[1]
