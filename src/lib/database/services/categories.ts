...
export class CategoriesService {
   ...
   */
  static async getAllCategories(includeEmpty: boolean = true): Promise<CategoryWithStats[]> {
    try {
   ...
   */
  static async updateCategory(id: number, data: Partial<CategoryData>): Promise<Category> {
    try {
  ...
  static async deleteCategory(id: number): Promise<void> {
    try {
      ...
      if (toolCount > 0) {
        throw new Error(`Cannot delete category with ${toolCount} tools. Reassign tools first.`)
      }
  ...
  static async getCategoryStatistics(): Promise<{
    ...
    try {
      ...
      const emptyCategoriesCount = categories.filter(cat => (cat.toolCount || 0) === 0).length
      const totalTools = categories.reduce((sum, cat) => sum + (cat.toolCount || 0), 0)
      const averageToolsPerCategory = totalCategories > 0 ? Math.round(totalTools / totalCategories) : 0
   ...
   */
  static async getRelatedCategories(categoryName: string, limit: number = 4): Promise<Category[]> {
    try {
