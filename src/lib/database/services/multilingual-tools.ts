  ...
  async searchTools(params: ToolsSearchParams): Promise<PaginatedToolsResult> {
    try {
      ...
      const processedTools: ToolWithTranslation[] = tools.map(tool => {
        const requestedTranslation = tool.translations?.find(
          t => t.languageCode === language
        );
        const fallbackTranslation = tool.translations?.find(
      ...
      throw new Error(
        `Failed to search tools: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async getFeaturedTools(
    ...
    try {
      ...
      const tools = await prisma.tool.findMany({
        ...
        take: limit,
        orderBy: [{ quality_score: 'desc' }, { createdAt: 'desc' }],
        include: {
          translations: {
            where: {
              languageCode: {
