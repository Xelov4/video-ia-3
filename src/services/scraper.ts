...
export async function runFullAnalysis(url: string): Promise<ToolAnalysis> {
  ...
  try {
    ...
    console.log(
      `✅ Scraping completed: ${scrapingData.content.length} chars, ${scrapingData.features.length} features`
    );
    ...
    console.log(
      `✅ Enhanced AI analysis completed: ${analysis.toolName} (${analysis.confidence}% confidence)`
    );
    ...
    console.log(
      `✅ Enhanced affiliate analysis completed: ${affiliateInfo.hasAffiliateProgram ? 'Program found' : 'No program'}`
    );
    ...
    console.log(
      `✅ Quality assessment completed: ${analysis.completenessScore}% complete`
    );

    console.log('🎉 Professional analysis complete!');
    console.log(
      `📊 Final results: ${analysis.toolName} - ${analysis.category} - ${analysis.confidence}% confidence`
    );
