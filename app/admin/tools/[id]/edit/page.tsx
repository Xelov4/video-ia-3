...
export default function AdminToolEditPage({ params }: PageProps) {
  ...
  const fetchToolData = useCallback(async (id: number) => {
    try {
      ...
      setError(
        error instanceof Error ? error.message : "Erreur lors du chargement de l'outil"
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!toolResponse.ok)
        throw new Error(
          (await toolResponse.json()).error || "Erreur lors de la sauvegarde de l'outil"
        );

      const translationPromises = Object.values(translations).map(t => {
        const endpoint = t.id
          ? `/api/tools/${toolId}/translations/${t.id}`
      ...
      if (failed > 0)
        toast.error(`Outil sauvegardé, mais ${failed} traduction(s) ont échoué.`);
      else {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error((await response.json()).error || 'Erreur de suppression');
        ...
        <div className='mb-8 rounded-lg bg-white p-6 shadow-md'>
          <div className='flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between'>
            <div>
              ...
              {hasUnsavedChanges && (
                <span className='mt-2 inline-flex items-center rounded-full bg-orange-100 px-2 py-1 text-xs font-medium text-orange-800'>
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      Cette action est irréversible et supprimera l'outil et ses
                      traductions.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
        ...
        <div className='space-y-8'>
          <Card className='shadow-md'>
            <CardHeader>
              ...
              <CardDescription>
                Modifiez ici les informations de base de l'outil. Ces informations
                serviront de source pour les traductions.
              </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <TabsContent value='general' className='pt-6'>
                  <div className='grid grid-cols-1 gap-6 md:grid-cols-2'>
                    <div>
                      ...
                      <Input
                        value={tool.toolName}
                        onChange={e => handleToolChange({ toolName: e.target.value })}
                    ...
                    <div>
                      ...
                      <Select
                        ...
                        onValueChange={value =>
                          handleToolChange({ toolCategory: value })
                        }
                      >
                        <SelectTrigger>
                    ...
                    <div>
                      ...
                      <Input
                        value={tool.toolLink}
                        onChange={e => handleToolChange({ toolLink: e.target.value })}
                        className='bg-slate-50'
                      />
                    </div>
                    <div>
                  ...
                  <div className='space-y-6'>
                    ...
                    <div>
                      <label className='font-medium'>
                        Public Cible (séparé par virgules)
                      </label>
                      <Input
                        value={tool.targetAudience}
                        onChange={e =>
                          handleToolChange({ targetAudience: e.target.value })
                        }
                        className='bg-slate-50'
                      />
                    </div>
                    <div>
                      <label className='font-medium'>
                        Fonctionnalités Clés (séparé par virgules)
                      </label>
                      <Input
                        value={tool.keyFeatures}
                        onChange={e =>
                          handleToolChange({ keyFeatures: e.target.value })
                        }
                        className='bg-slate-50'
                      />
                    </div>
                    <div>
                      <label className='font-medium'>
                        Cas d'usage (séparé par virgules)
                      </label>
                      <Input
                        value={tool.useCases}
                        onChange={e => handleToolChange({ useCases: e.target.value })}
                        className='bg-slate-50'
                      />
                    </div>
                  </div>
                  ...
                  <div className='grid grid-cols-1 gap-6 md:grid-cols-2'>
                    <div>
                      ...
                      <Input
                        ...
                        onChange={e => handleToolChange({ metaTitle: e.target.value })}
                        className='bg-slate-50'
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        value={tool.metaDescription}
                        onChange={e =>
                          handleToolChange({ metaDescription: e.target.value })
                        }
                        className='bg-slate-50'
                      />
                    </div>
                    <div className='md:col-span-2'>
                      <label className='font-medium'>
                        Mots-clés SEO (séparé par virgules)
                      </label>
                      <Input
                        value={tool.seoKeywords}
                        onChange={e =>
                          handleToolChange({ seoKeywords: e.target.value })
                        }
                        className='bg-slate-50'
                      />
                    </div>
                ...
                <TabsContent value='visibility' className='pt-6'>
                  <div className='grid grid-cols-1 gap-6 md:grid-cols-2'>
                    <div>
                      ...
                      <Input
                        value={tool.imageUrl}
                        onChange={e => handleToolChange({ imageUrl: e.target.value })}
                        className='bg-slate-50'
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        type='number'
                        value={tool.quality_score}
                        onChange={e =>
                          handleToolChange({ quality_score: parseInt(e.target.value) })
                        }
                        className='bg-slate-50'
                      />
                    </div>
                    <div className='flex items-center space-x-2'>
                      <Switch
                        ...
                        onCheckedChange={c => handleToolChange({ isActive: c })}
                      />
                    ...
                    <div className='flex items-center space-x-2'>
                      <Switch
                        ...
                        onCheckedChange={c => handleToolChange({ featured: c })}
                      />
