...
export default function AdminArticlesPage() {
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            {row.status === 'PUBLISHED' && (
              <DropdownMenuItem asChild>
                <Link href={`/fr/blog/${row.slug}`} target="_blank" rel="noopener noreferrer">
  ...
  }, [pathname, router, state.currentPage, state.pageSize, state.sortColumn, state.sortDirection, debouncedSearch, state.filters.status, state.filters.postType, state.filters.featured, state.filters.hasComments, state.filters.minViews])

  useEffect(() => {
    if (session) loadArticles()
  }, [session, state.currentPage, state.pageSize, state.sortColumn, state.sortDirection, debouncedSearch, state.filters.status, state.filters.postType, state.filters.featured, state.filters.hasComments, state.filters.minViews])
  ...
  const loadArticles = async () => {
    try {
      ...
      const articles: Article[] = (data.data?.posts || []).map((post: any) => ({
        ...
        author: post.author,
        translation: (post.translations && post.translations[0]) || { title: '', content: '', excerpt: null },
  ...
  const handleDeleteArticle = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/posts?id=${deleteArticleId}`, { method: 'DELETE' })
      if (!res.ok) {
        const err = await res.json().catch(() => ({}))
