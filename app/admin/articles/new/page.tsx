...
export default function NewArticlePage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const failedTranslations = translationResults.filter(r => 
        r.status === 'rejected' || (r.status === 'fulfilled' && !r.value.ok)
      ).length

      if (failedTranslations > 0) {
        toast.warning(`Article créé, mais ${failedTranslations} traduction(s) ont échoué`)
      } else {
        ...
        <div className="space-y-8">
          ...
          <Card className="shadow-md">
            <CardHeader>
              ...
              <CardDescription>
                Configuration de base de l'article
              </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <TabsContent value="general" className="pt-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      />
                      <p className="text-sm text-gray-500 mt-1">URL finale: /blog/{postData.slug}</p>
                    </div>
                    <div>
                      <label className="font-medium">Type d'article</label>
                      <Select value={postData.postType} onValueChange={value => setPostData(prev => ({ ...prev, postType: value as any }))}>
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                    ...
                    <div>
                      ...
                      <Select value={postData.status} onValueChange={value => setPostData(prev => ({ ...prev, status: value as any }))}>
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                    ...
                    <div>
                      ...
                      <Input 
                        ...
                        onChange={e => setPostData(prev => ({ ...prev, readingTimeMinutes: e.target.value ? parseInt(e.target.value) : undefined }))}
                        placeholder="5"
                        className="bg-slate-50" 
                      />
                    </div>
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <Input 
                        ...
                        onChange={e => setPostData(prev => ({ ...prev, featuredImageUrl: e.target.value }))}
