...
const CategoryCard = ({
  ...
  return (
    <Card
      className='group cursor-pointer border-0 bg-white shadow-md transition-all duration-300 hover:-translate-y-1 hover:shadow-lg'
      onClick={onClick}
    >
      <CardHeader className='pb-3'>
        <div className='mb-2 flex items-center justify-between'>
          <div className='flex items-center space-x-3'>
            <div className='flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-blue-500 to-purple-600 text-xl font-bold text-white'>
              {category.emoji || category.displayName.charAt(0).toUpperCase()}
            </div>
            <div>
              <CardTitle className='line-clamp-1 text-sm transition-colors group-hover:text-primary md:text-base'>
      ...
      <CardContent className='pt-0'>
        <p className='line-clamp-2 text-xs leading-relaxed text-muted-foreground md:text-sm'>
          {category.displayDescription ||
            ...
            `Découvrez les meilleurs outils de ${category.displayName.toLowerCase()}`}
        </p>
      </CardContent>
    </Card>
  );
...
const TopCategoriesCarousel = ({
  ...
  const topCategories = categories
    .sort(
      (a, b) =>
        (b.actualToolCount || b.toolCount || 0) -
        (a.actualToolCount || a.toolCount || 0)
    )
    .slice(0, 12);
  ...
  return (
    <div className='mb-8 md:mb-12'>
      <div className='mb-4 flex items-center justify-between md:mb-6'>
        <h2 className='text-xl font-bold md:text-2xl'>{t.popularCategories}</h2>
      ...
      <div className='overflow-x-auto pb-4'>
        <div className='flex w-max space-x-4 md:space-x-6'>
          {topCategories.map(category => (
            <div key={category.id} className='w-72 flex-none md:w-80'>
              <CategoryCard
                category={category}
                lang={lang}
                t={t}
      ...
      <div className='container mx-auto px-4 py-8 md:py-12'>
            ...
            >
              ...
              <SelectContent>
                <SelectItem value='count-desc'>
                  {t.sortCount} ({t.orderDesc})
                </SelectItem>
                <SelectItem value='count-asc'>
                  {t.sortCount} ({t.orderAsc})
                </SelectItem>
        ...
        {viewMode === 'grid' ? (
          ...
          <div className='space-y-8'>
            {categoriesByLetter
              .slice(0, Math.ceil(visibleCount / 24) * categoriesByLetter.length)
              .map(({ letter, categories: letterCategories }) => (
                <div key={letter}>
                  <div className='mb-4 flex items-center'>
                    <div className='mr-4 flex h-12 w-12 items-center justify-center rounded-xl bg-primary text-xl font-bold text-primary-foreground'>
                      {letter}
                    </div>
