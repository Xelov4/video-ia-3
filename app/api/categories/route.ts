...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const limit = searchParams.get('limit')
      ? validateLimitParam(searchParams.get('limit')!, 100)
      : undefined;
    ...
    if (featured) {
      ...
      const categories = await multilingualCategoriesService.getFeaturedCategories(
        language,
        limit || 8
      );
      result = {
        ...
        meta: {
          ...
          fallbackCount: categories.filter(cat => cat.translationSource !== 'exact')
            .length,
      ...
      const categories = await multilingualCategoriesService.searchCategories(
        search,
        language,
        limit || 50
      );
      ...
      // Get all categories
      const allCategoriesResult = await multilingualCategoriesService.getAllCategories(
        language,
        {
          includeEmpty,
          useCache,
      ...
      result = {
        ...
        meta: {
          language,
          totalCount:
            allCategoriesResult.categories?.length || allCategoriesResult?.length || 0,
          fallbackCount: 0,
          cacheHit: false,
        },
      };
    ...
    const totalTools = result.categories.reduce(
      (sum, cat) => sum + (cat.actualToolCount || 0),
      0
    );
    ...
    responseHeaders.set(
      I18N_HEADERS.FALLBACK_USED,
      result.meta.fallbackCount.toString()
    );
    responseHeaders.set(
      I18N_HEADERS.CACHE_STATUS,
      result.meta.cacheHit ? 'HIT' : 'MISS'
    );
