...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('📖 Getting published blog posts', { page, limit, category, tag, search, featured, language })
    ...
    const transformedPosts = result.posts.map(post => ({
      ...post,
      translation: post.translations.find(t => t.languageCode === language) || post.translations[0] || {}
    }))
