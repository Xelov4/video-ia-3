...
async function testJsonFix() {
  ...
  try {
    ...
    if (!process.env.GEMINI_API_KEY) {
      console.log('⚠️  GEMINI_API_KEY non configurée - Test en mode simulation uniquement')
    } else {
    ...
    for (const testCase of testCases) {
      try {
        ...
        const result = service.cleanTranslationResponse.call(service, testCase.response, 'overview')
    ...
    if (!service.extractPartialTranslation) {
      ...
      try {
        ...
        const extracted = service.extractPartialTranslation.call(service, malformedResponse, 'TestTool', 'fr')
    ...
    if (process.env.GEMINI_API_KEY) {
      ...
      try {
        ...
        console.log('💡 Cela peut être normal si la clé API est invalide ou les quotas dépassés')
      }
    }
    ...
    console.log(`\n📋 RÉSUMÉ DES TESTS:`)
    console.log(`   🧪 Test 1 (cleanTranslationResponse): ${successCount}/${totalTests} ✅`)
    console.log(`   🧪 Test 2 (extraction partielle): ${service.extractPartialTranslation ? '✅ Disponible' : '❌ Non disponible'}`)
    console.log(`   🧪 Test 3 (API Gemini): ${process.env.GEMINI_API_KEY ? '✅ Configuré' : '⚠️ Non configuré'}`)
