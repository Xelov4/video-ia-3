...
async function testTranslationsOnly() {
  ...
  try {
    ...
    console.log(
      `🌐 6 langues: Français, Italien, Espagnol, Allemand, Néerlandais, Portugais`
    );
    console.log(`\n${'='.repeat(60)}\n`);
    ...
    const mockEnglishContent = {
      overview:
        'Visualizee is an AI rendering tool that transforms sketches, images, 3D models, or text prompts into realistic visualizations. It allows users to create renders quickly, without needing expensive 3D software.',
...
Visualizee streamlines the visualization process for architects, interior designers, product developers, and marketing teams who need quick, professional results without the complexity of traditional 3D rendering software.`,
      ...
      metaDescription:
        'Turn sketches & ideas into realistic 3D renders in seconds with AI! Get 3 free renders. No credit card needed. Try Visualizee now!',
...
• Visualizee helps you generate 3D visualizations from various 3D model formats like FBX, OBJ, and GLTF`,
    ...
    console.log(
      `   Taux de réussite: ${((result.successfulTranslations / result.totalLanguages) * 100).toFixed(1)}%`
    );
    ...
    languages.forEach(lang => {
      ...
      if (translation && !translation.error) {
        ...
        console.log(
          `     📏 Overview: ${translation.overview?.length || 0} caractères`
        );
        console.log(
          `     📏 Description: ${translation.description?.length || 0} caractères`
        );
        console.log(
          `     📏 Meta Title: ${translation.metaTitle?.length || 0} caractères`
        );
        console.log(
          `     📏 Meta Description: ${translation.metaDescription?.length || 0} caractères`
        );
        console.log(
          `     📏 Key Features: ${translation.keyFeatures ? (translation.keyFeatures.match(/•/g) || []).length : 0} bullet points`
        );
        console.log(
          `     📏 Use Cases: ${translation.useCases ? (translation.useCases.match(/•/g) || []).length : 0} bullet points`
        );
        console.log(
          `     📏 Target Audience: ${translation.targetAudience?.length || 0} caractères`
        );

        // Vérifications qualité
        const hasVideoIA = translation.metaTitle?.includes('- Video-IA.net');
        const overviewSentences = translation.overview
          ? translation.overview.split(/[.!?]+/).filter(s => s.trim().length > 0).length
          : 0;
        const targetSentences = translation.targetAudience
          ? translation.targetAudience.split(/[.!?]+/).filter(s => s.trim().length > 0)
              .length
          : 0;

        console.log(`     🔍 Qualité:`);
        console.log(
          `       ${hasVideoIA ? '✅' : '❌'} Meta title avec "- Video-IA.net"`
        );
        console.log(
          `       ${overviewSentences === 2 ? '✅' : '⚠️'} Overview (${overviewSentences} phrases, attendu: 2)`
        );
        console.log(
          `       ${targetSentences >= 3 && targetSentences <= 4 ? '✅' : '⚠️'} Target Audience (${targetSentences} phrases, attendu: 3-4)`
        );
      } else {
