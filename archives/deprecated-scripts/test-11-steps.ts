...
async function testComplete11Steps() {
  ...
  try {
    ...
    if (result.socialLinks && Object.keys(result.socialLinks).length > 0) {
      console.log(`   ✅ ${Object.keys(result.socialLinks).length} réseaux sociaux validés par Gemini:`)
    ...
    if (result.usefulLinks && Object.keys(result.usefulLinks).length > 0) {
      console.log(`   ✅ ${Object.keys(result.usefulLinks).length} liens utiles validés par Gemini:`)
      Object.entries(result.usefulLinks).forEach(([type, url]) => {
        const linkEmoji = { mailAddress: '📧', docsLink: '📚', affiliateLink: '🤝', changelogLink: '📝' }
        const typeName = { mailAddress: 'Email', docsLink: 'Documentation', affiliateLink: 'Affiliation', changelogLink: 'Changelog' }
    ...
    if (result.generatedContent) {
      ...
      console.log(`   ✅ Article généré: ${result.generatedContent.length} chars, ${wordCount} mots`)
      console.log(`   📋 Structure: ${h2Count} sections H2`)
      console.log(`   ${result.generatedContent.includes("What's ") ? '✅' : '❌'} Titre requis présent`)
    } else {
    ...
    if (result.generatedOverview) {
      const sentenceCount = result.generatedOverview.split(/[.!?]+/).filter(s => s.trim().length > 0).length
      console.log(`   ✅ Overview généré: ${result.generatedOverview.length} chars`)
      console.log(`   📝 "${result.generatedOverview}"`)
      console.log(`   ${sentenceCount === 2 ? '✅' : '❌'} Exactement 2 phrases: ${sentenceCount}`)
    } else {
    ...
    if (result.generatedMetaTitle && result.generatedMetaDescription) {
      ...
      console.log(`   📝 Title: "${result.generatedMetaTitle}" (${result.generatedMetaTitle.length}/70 chars)`)
      console.log(`   📝 Description: "${result.generatedMetaDescription}" (${result.generatedMetaDescription.length}/160 chars)`)
      console.log(`   ${result.generatedMetaTitle.includes('- Video-IA.net') ? '✅' : '❌'} Suffix Video-IA.net présent`)
    } else {
    ...
    if (result.generatedPricingModel) {
      ...
      console.log(`   ${validModels.includes(result.generatedPricingModel) ? '✅' : '❌'} Modèle valide`)
    } else {
    ...
    if (result.generatedUseCases) {
      ...
      console.log(`   ${bulletCount >= 3 && bulletCount <= 4 ? '✅' : '⚠️'} Nombre optimal (3-4): ${bulletCount}`)
    } else {
    ...
    if (result.generatedTargetAudience) {
      const sentenceCount = result.generatedTargetAudience.split(/[.!?]+/).filter(s => s.trim().length > 0).length
      console.log(`   ✅ Target audience généré: ${result.generatedTargetAudience.length} chars`)
      console.log(`   📝 Contenu: "${result.generatedTargetAudience.substring(0, 200)}..."`)
      console.log(`   ${sentenceCount >= 3 && sentenceCount <= 4 ? '✅' : '⚠️'} Sentences (3-4): ${sentenceCount}`)
    } else {
    ...
    const detailedReport = {
      ...
      stepByStepResults: {
        ...
        step6_overview: {
          ...
          success: result.generatedOverview && result.generatedOverview.split(/[.!?]+/).filter(s => s.trim().length > 0).length === 2,
          data: { overviewLength: result.generatedOverview ? result.generatedOverview.length : 0, sentences: result.generatedOverview ? result.generatedOverview.split(/[.!?]+/).filter(s => s.trim().length > 0).length : 0 }
        },
        step7_keyFeatures: {
        ...
        step11_targetAudience: {
          ...
          success: result.generatedTargetAudience && result.generatedTargetAudience.length >= 200 && result.generatedTargetAudience.split(/[.!?]+/).filter(s => s.trim().length > 0).length >= 3,
          data: { textLength: result.generatedTargetAudience ? result.generatedTargetAudience.length : 0, sentences: result.generatedTargetAudience ? result.generatedTargetAudience.split(/[.!?]+/).filter(s => s.trim().length > 0).length : 0 }
        }
      },
      fullResults: result,
      summary: {
        ...
        stepsSuccessful: Object.values({
          ...
          step6: result.generatedOverview && result.generatedOverview.split(/[.!?]+/).filter(s => s.trim().length > 0).length === 2,
        ...
        dataQuality: {
          ...
          properOverview: result.generatedOverview && result.generatedOverview.split(/[.!?]+/).filter(s => s.trim().length > 0).length === 2,
    ...
    console.log(`\n🔍 RÉSULTAT DÉTAILLÉ:`)
    console.log(`   ${detailedReport.stepByStepResults.step1_httpCheck.success ? '✅' : '❌'} HTTP Check`)
    console.log(`   ${detailedReport.stepByStepResults.step1_5_screenshot.success ? '✅' : '❌'} Screenshot WebP`)
    console.log(`   ${detailedReport.stepByStepResults.step2_crawling.success ? '✅' : '❌'} Crawling (${result.processedPages || 0} pages)`)
    console.log(`   ${detailedReport.stepByStepResults.step3_socialLinks.success ? '✅' : '❌'} Social + Validation (${detailedReport.stepByStepResults.step3_socialLinks.data.linksFound})`)
    console.log(`   ${detailedReport.stepByStepResults.step4_usefulLinks.success ? '✅' : '❌'} Useful + Validation (${detailedReport.stepByStepResults.step4_usefulLinks.data.linksFound})`)
    console.log(`   ${detailedReport.stepByStepResults.step5_contentGeneration.success ? '✅' : '❌'} Content (${detailedReport.stepByStepResults.step5_contentGeneration.data.wordCount} mots)`)
    console.log(`   ${detailedReport.stepByStepResults.step6_overview.success ? '✅' : '❌'} Overview (${detailedReport.stepByStepResults.step6_overview.data.sentences} phrases)`)
    console.log(`   ${detailedReport.stepByStepResults.step7_keyFeatures.success ? '✅' : '❌'} Key Features (${detailedReport.stepByStepResults.step7_keyFeatures.data.bulletPoints} bullets)`)
    console.log(`   ${detailedReport.stepByStepResults.step8_metaData.success ? '✅' : '❌'} Meta + Video-IA.net`)
    console.log(`   ${detailedReport.stepByStepResults.step9_pricingModel.success ? '✅' : '❌'} Pricing (${result.generatedPricingModel || 'N/A'})`)
    console.log(`   ${detailedReport.stepByStepResults.step10_useCases.success ? '✅' : '❌'} Use Cases (${detailedReport.stepByStepResults.step10_useCases.data.bulletPoints} bullets)`)
    console.log(`   ${detailedReport.stepByStepResults.step11_targetAudience.success ? '✅' : '❌'} Target Audience (${detailedReport.stepByStepResults.step11_targetAudience.data.sentences} phrases)`)
    
    console.log(`\n📊 QUALITÉ GLOBALE:`)
    const qualityChecks = Object.values(detailedReport.summary.dataQuality).filter(Boolean).length
    console.log(`   ${qualityChecks >= 8 ? '🏆' : qualityChecks >= 6 ? '✅' : '⚠️'} Score qualité: ${qualityChecks}/11`)
