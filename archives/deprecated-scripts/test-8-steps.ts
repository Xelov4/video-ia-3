...
async function testComplete8Steps() {
  ...
  try {
    ...
    if (result.socialLinks && Object.keys(result.socialLinks).length > 0) {
      console.log(`   ✅ ${Object.keys(result.socialLinks).length} réseaux sociaux validés:`)
      ...
      console.log(`   🤖 Validation Gemini: Liens vérifiés comme pertinents pour ${result.toolName}`)
    } else {
      console.log(`   ❌ Aucun réseau social validé`)
      console.log(`   💡 Les patterns recherchent puis Gemini valide: LinkedIn, Facebook, X/Twitter, GitHub, Discord, Instagram, TikTok`)
    }
    
    console.log(`\n🔗 ÉTAPE 4 - EXTRACTION LIENS UTILES (AVEC VALIDATION GEMINI):`)
    if (result.usefulLinks && Object.keys(result.usefulLinks).length > 0) {
      console.log(`   ✅ ${Object.keys(result.usefulLinks).length} liens utiles validés:`)
      ...
      console.log(`   🤖 Validation Gemini: Liens vérifiés comme spécifiques à ${result.toolName}`)
    } else {
      console.log(`   ❌ Aucun lien utile validé`)
      console.log(`   💡 Recherche puis validation: emails, docs, liens d'affiliation, changelogs`)
    }
    ...
    if (result.generatedOverview) {
      ...
      console.log(`   ${result.generatedOverview.length <= 150 ? '✅' : '⚠️'} Longueur optimale (≤150 chars): ${result.generatedOverview.length <= 150}`)
      const sentenceCount = result.generatedOverview.split(/[.!?]+/).filter(s => s.trim().length > 0).length
      console.log(`   ${sentenceCount <= 2 ? '✅' : '⚠️'} Nombre de phrases (≤2): ${sentenceCount}`)
    } else {
    ...
    if (result.generatedKeyFeatures) {
      ...
      console.log(`   ${bulletCount >= 3 && bulletCount <= 6 ? '✅' : '⚠️'} Nombre de bullet points (3-6): ${bulletCount}`)
    } else {
    ...
    if (result.generatedMetaTitle && result.generatedMetaDescription) {
      ...
      console.log(`   ${result.generatedMetaTitle.length <= 60 ? '✅' : '⚠️'} Longueur title optimale (≤60): ${result.generatedMetaTitle.length <= 60}`)
      console.log(`   📝 Meta Description: "${result.generatedMetaDescription}"`)
      console.log(`   📏 Longueur description: ${result.generatedMetaDescription.length} chars`)
      console.log(`   ${result.generatedMetaDescription.length <= 160 ? '✅' : '⚠️'} Longueur description optimale (≤160): ${result.generatedMetaDescription.length <= 160}`)
    } else {
    ...
    const detailedReport = {
      ...
      stepByStepResults: {
        ...
        step6_overviewGeneration: {
          ...
          data: {
            overviewLength: result.generatedOverview ? result.generatedOverview.length : 0,
            isOptimalLength: result.generatedOverview ? result.generatedOverview.length <= 150 : false,
            sentenceCount: result.generatedOverview ? result.generatedOverview.split(/[.!?]+/).filter(s => s.trim().length > 0).length : 0
          }
        },
        step7_keyFeaturesGeneration: {
          ...
          data: {
            keyFeaturesLength: result.generatedKeyFeatures ? result.generatedKeyFeatures.length : 0,
            bulletPointCount: result.generatedKeyFeatures ? (result.generatedKeyFeatures.match(/•/g) || []).length : 0,
            isOptimalCount: result.generatedKeyFeatures ? (result.generatedKeyFeatures.match(/•/g) || []).length >= 3 && (result.generatedKeyFeatures.match(/•/g) || []).length <= 6 : false
          }
        },
        step8_metaGeneration: {
          ...
          success: !!(result.generatedMetaTitle && result.generatedMetaDescription && result.generatedMetaTitle.length <= 60 && result.generatedMetaDescription.length <= 160),
          data: {
            metaTitleLength: result.generatedMetaTitle ? result.generatedMetaTitle.length : 0,
            metaDescriptionLength: result.generatedMetaDescription ? result.generatedMetaDescription.length : 0,
            titleOptimal: result.generatedMetaTitle ? result.generatedMetaTitle.length <= 60 : false,
            descriptionOptimal: result.generatedMetaDescription ? result.generatedMetaDescription.length <= 160 : false
          }
        }
      },
      fullResults: result,
      summary: {
        ...
        stepsCompleted: result.step === 'completed' ? 8 : ['http_check', 'crawling', 'social_extraction', 'useful_links', 'content_generation', 'overview_generation', 'keyfeatures_generation', 'meta_generation'].indexOf(result.step) + 1,
    ...
    console.log(`   ${detailedReport.stepByStepResults.step3_socialExtraction.success ? '✅' : '❌'} Social Links + Validation (${detailedReport.stepByStepResults.step3_socialExtraction.data.linksFound} validés)`)
    console.log(`   ${detailedReport.stepByStepResults.step4_usefulLinks.success ? '✅' : '❌'} Useful Links + Validation (${detailedReport.stepByStepResults.step4_usefulLinks.data.linksFound} validés)`)
    ...
    console.log(`   ${detailedReport.stepByStepResults.step6_overviewGeneration.success ? '✅' : '❌'} Overview Generation (${detailedReport.stepByStepResults.step6_overviewGeneration.data.overviewLength} chars)`)
    console.log(`   ${detailedReport.stepByStepResults.step7_keyFeaturesGeneration.success ? '✅' : '❌'} Key Features Generation (${detailedReport.stepByStepResults.step7_keyFeaturesGeneration.data.bulletPointCount} bullets)`)
    console.log(`   ${detailedReport.stepByStepResults.step8_metaGeneration.success ? '✅' : '❌'} Meta Generation (${detailedReport.stepByStepResults.step8_metaGeneration.data.metaTitleLength}/${detailedReport.stepByStepResults.step8_metaGeneration.data.metaDescriptionLength} chars)`)
    ...
    console.log(`   ${detailedReport.summary.dataQuality.hasOverview ? '✅' : '❌'} Overview généré`)
    console.log(`   ${detailedReport.summary.dataQuality.hasKeyFeatures ? '✅' : '❌'} Key features générées`)
    console.log(`   ${detailedReport.summary.dataQuality.hasMetaData ? '✅' : '❌'} Meta données générées`)
