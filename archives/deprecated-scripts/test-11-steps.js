    ...
    return __generator(this, function (_a) {
      switch (_a.label) {
        ...
        case 1:
          ...
          return [
            4 /*yield*/,
            toolContentUpdater_1.ToolContentUpdaterService.updateToolContent(
              toolId,
              true
            ),
          ];
        case 2:
          ...
          if (result.httpStatusCode) {
            ...
            console.log(
              '   '
                .concat(result.isActive ? '🟢' : '🔴', ' Status actif: ')
                .concat(result.isActive)
            );
          } else {
          ...
          if (result.screenshotPath) {
            ...
            console.log(
              '   \uD83D\uDDBC\uFE0F  Format: WebP optimis\u00E9 pour le web'
            );
          } else {
          ...
          if (result.processedPages !== undefined) {
            console.log(
              '   \uD83D\uDCC4 Pages crawl\u00E9es: '.concat(result.processedPages)
            );
            console.log(
              '   \uD83D\uDCCA Efficacit\u00E9: '.concat(
          ...
          if (result.socialLinks && Object.keys(result.socialLinks).length > 0) {
            console.log(
              '   \u2705 '.concat(
                ...
                ' r\u00E9seaux sociaux valid\u00E9s par Gemini:'
              )
            );
            Object.entries(result.socialLinks).forEach(function (_a) {
              ...
              console.log(
                '     '
                  .concat(platformEmoji[platform] || '🔗', ' ')
                  .concat(
                    platform.replace('social', '').charAt(0).toUpperCase() +
                      platform.replace('social', '').slice(1),
                    ': '
                  )
                  .concat(url)
              );
          ...
          if (result.usefulLinks && Object.keys(result.usefulLinks).length > 0) {
            console.log(
              '   \u2705 '.concat(
                Object.keys(result.usefulLinks).length,
                ' liens utiles valid\u00E9s par Gemini:'
              )
            );
            Object.entries(result.usefulLinks).forEach(function (_a) {
              ...
              console.log(
                '     '
                  .concat(linkEmoji[type] || '🔗', ' ')
                  .concat(typeName[type] || type, ': ')
                  .concat(url)
              );
            });
          } else {
          ...
          if (result.generatedOverview) {
            ...
            console.log(
              '   '
                .concat(sentenceCount === 2 ? '✅' : '❌', ' Exactement 2 phrases: ')
                .concat(sentenceCount)
            );
          } else {
          ...
          if (result.generatedKeyFeatures) {
            ...
            console.log(
              '   \u2705 Key features g\u00E9n\u00E9r\u00E9es: '.concat(
                bulletCount,
                ' bullet points'
              )
            );
            result.generatedKeyFeatures.split('\n').forEach(function (line) {
              if (line.trim()) console.log('     '.concat(line));
            });
          } else {
          ...
          if (result.generatedMetaTitle && result.generatedMetaDescription) {
            ...
            console.log(
              '   \uD83D\uDCDD Title: "'
                .concat(result.generatedMetaTitle, '" (')
                .concat(result.generatedMetaTitle.length, '/70 chars)')
            );
            console.log(
              '   \uD83D\uDCDD Description: "'
                .concat(result.generatedMetaDescription, '" (')
                .concat(result.generatedMetaDescription.length, '/160 chars)')
            );
            console.log(
              '   '.concat(
                result.generatedMetaTitle.includes('- Video-IA.net') ? '✅' : '❌',
                ' Suffix Video-IA.net pr\u00E9sent'
              )
            );
          } else {
          ...
          if (result.generatedPricingModel) {
            ...
            console.log(
              '   '.concat(
                validModels.includes(result.generatedPricingModel) ? '✅' : '❌',
                ' Mod\u00E8le valide'
              )
            );
          } else {
          ...
          if (result.generatedTargetAudience) {
            sentenceCount = result.generatedTargetAudience
              .split(/[.!?]+/)
              .filter(function (s) {
                return s.trim().length > 0;
              }).length;
            console.log(
              '   \u2705 Target audience g\u00E9n\u00E9r\u00E9: '.concat(
            ...
            console.log(
              '   \uD83D\uDCDD Contenu: "'.concat(
                result.generatedTargetAudience.substring(0, 200),
                '..."'
              )
            );
            console.log(
              '   '
                .concat(
                  sentenceCount >= 3 && sentenceCount <= 4 ? '✅' : '⚠️',
                  ' Sentences (3-4): '
                )
                .concat(sentenceCount)
            );
          ...
          if (result.errors && result.errors.length > 0) {
            console.log(
              '   \u26A0\uFE0F  '.concat(result.errors.length, ' erreur(s):')
            );
          ...
          detailedReport = {
            ...
            stepByStepResults: {
              ...
              step4_usefulLinks: {
                ...
                success:
                  result.usefulLinks && Object.keys(result.usefulLinks).length > 0,
                data: {
                  linksFound: result.usefulLinks
                    ? Object.keys(result.usefulLinks).length
                    : 0,
              ...
              step5_contentGeneration: {
                ...
                data: {
                  contentLength: result.generatedContent
              ...
              step6_overview: {
                ...
                success:
                  ...
                  result.generatedOverview.split(/[.!?]+/).filter(function (s) {
                    return s.trim().length > 0;
                  }).length === 2,
                data: {
              ...
              step7_keyFeatures: {
                ...
                success:
                  ...
                  result.generatedKeyFeatures.includes('•'),
                data: {
              ...
              step8_metaData: {
                ...
                completed: !!(
                  result.generatedMetaTitle && result.generatedMetaDescription
                ),
                success: !!(
              ...
              step11_targetAudience: {
                ...
                success:
                  ...
                  result.generatedTargetAudience.split(/[.!?]+/).filter(function (s) {
                    return s.trim().length > 0;
                  }).length >= 3,
                data: {
            ...
            summary: {
              overallSuccess:
                result.status === 'success' && result.step === 'completed',
              stepsCompleted:
              ...
              stepsSuccessful: Object.values({
                ...
                step10:
                  result.generatedUseCases && result.generatedUseCases.includes('•'),
                step11:
                  result.generatedTargetAudience &&
                  result.generatedTargetAudience.length >= 200,
              ...
              dataQuality: {
                ...
                documentationAvailable:
                  result.usefulLinks && result.usefulLinks.docsLink,
                completeContent:
          ...
          return [
            4 /*yield*/,
            fs.writeFile(reportFilename, JSON.stringify(detailedReport, null, 2)),
          ];
        case 3:
          _a.sent();
          console.log('\n'.concat('='.repeat(80)));
          ...
          console.log(
            '\uD83D\uDD04 \u00C9tapes compl\u00E9t\u00E9es: '.concat(
              detailedReport.summary.stepsCompleted,
              '/11'
            )
          );
          console.log(
            '\uD83C\uDFC6 \u00C9tapes r\u00E9ussies: '.concat(
              detailedReport.summary.stepsSuccessful,
              '/11'
            )
          );
          console.log('\n\uD83D\uDD0D R\u00C9SULTAT D\u00C9TAILL\u00C9:');
          console.log(
            '   '.concat(
              detailedReport.stepByStepResults.step1_httpCheck.success ? '✅' : '❌',
              ' HTTP Check'
            )
          );
          console.log(
            '   '.concat(
              detailedReport.stepByStepResults.step1_5_screenshot.success ? '✅' : '❌',
              ' Screenshot WebP'
            )
          );
          console.log(
            '   '
              .concat(
                detailedReport.stepByStepResults.step2_crawling.success ? '✅' : '❌',
                ' Crawling ('
              )
              .concat(result.processedPages || 0, ' pages)')
          );
          console.log(
            '   '
              .concat(
                detailedReport.stepByStepResults.step3_socialLinks.success
                  ? '✅'
                  : '❌',
                ' Social + Validation ('
              )
              .concat(
                detailedReport.stepByStepResults.step3_socialLinks.data.linksFound,
                ')'
              )
          );
          console.log(
            '   '
              .concat(
                detailedReport.stepByStepResults.step4_usefulLinks.success
                  ? '✅'
                  : '❌',
                ' Useful + Validation ('
              )
              .concat(
                detailedReport.stepByStepResults.step4_usefulLinks.data.linksFound,
                ')'
              )
          );
          console.log(
            '   '
              .concat(
                detailedReport.stepByStepResults.step5_contentGeneration.success
                  ? '✅'
                  : '❌',
                ' Content ('
              )
              .concat(
                detailedReport.stepByStepResults.step5_contentGeneration.data.wordCount,
                ' mots)'
              )
          );
          console.log(
            '   '
              .concat(
                detailedReport.stepByStepResults.step6_overview.success ? '✅' : '❌',
                ' Overview ('
              )
              .concat(
                detailedReport.stepByStepResults.step6_overview.data.sentences,
                ' phrases)'
              )
          );
          console.log(
            '   '
              .concat(
                detailedReport.stepByStepResults.step7_keyFeatures.success
                  ? '✅'
                  : '❌',
                ' Key Features ('
              )
              .concat(
                detailedReport.stepByStepResults.step7_keyFeatures.data.bulletPoints,
                ' bullets)'
              )
          );
          console.log(
            '   '.concat(
              detailedReport.stepByStepResults.step8_metaData.success ? '✅' : '❌',
              ' Meta + Video-IA.net'
            )
          );
          console.log(
            '   '
              .concat(
                detailedReport.stepByStepResults.step9_pricingModel.success
                  ? '✅'
                  : '❌',
                ' Pricing ('
              )
              .concat(result.generatedPricingModel || 'N/A', ')')
          );
          console.log(
            '   '
              .concat(
                detailedReport.stepByStepResults.step10_useCases.success ? '✅' : '❌',
                ' Use Cases ('
              )
              .concat(
                detailedReport.stepByStepResults.step10_useCases.data.bulletPoints,
                ' bullets)'
              )
          );
          console.log(
            '   '
              .concat(
                detailedReport.stepByStepResults.step11_targetAudience.success
                  ? '✅'
                  : '❌',
                ' Target Audience ('
              )
              .concat(
                detailedReport.stepByStepResults.step11_targetAudience.data.sentences,
                ' phrases)'
              )
          );
          console.log('\n\uD83D\uDCCA QUALIT\u00C9 GLOBALE:');
          qualityChecks = Object.values(detailedReport.summary.dataQuality).filter(
            Boolean
          ).length;
          console.log(
            '   '
              .concat(
                qualityChecks >= 8 ? '🏆' : qualityChecks >= 6 ? '✅' : '⚠️',
                ' Score qualit\u00E9: '
              )
              .concat(qualityChecks, '/11')
          );
          console.log('\n\uD83D\uDCBE Rapport JSON complet: '.concat(reportFilename));
          console.log(
            '\u274C Erreurs: '.concat(detailedReport.summary.hasErrors ? 'Oui' : 'Non')
          );
          if (
            ...
            console.log(
              '🚀 Le système 11 étapes génère un contenu complet et de qualité !'
            );
            ...
            console.log(
              '🔧 Le système fonctionne bien, optimisations mineures à considérer'
            );
          } else {
        ...
        case 4:
          ...
          if (error_1.code) console.log('   Code: '.concat(error_1.code));
          if (error_1.stack)
