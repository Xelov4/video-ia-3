...
export class ToolContentUpdaterService {
   ...
   */
  private static async validateLinksWithGemini(
    links: any,
    tool: Tool,
    linkType: 'social' | 'useful'
  ): Promise<any> {
    try {
...
{}`;
      ...
      try {
        ...
        if (jsonMatch) {
          ...
          console.log(
            `🤖 Gemini validation: ${Object.keys(links).length} -> ${Object.keys(validatedLinks).length} links`
          );
  ...
  static async extractSocialLinks(
    ...
    console.log(
      `🤖 Validation Gemini des ${Object.keys(socialLinks).length} liens sociaux...`
    );
    const validatedSocialLinks = await this.validateLinksWithGemini(
      socialLinks,
      tool,
      'social'
    );
  ...
  static async extractUsefulLinks(
    ...
    // Validation avec Gemini AI
    console.log(
      `🤖 Validation Gemini des ${Object.keys(usefulLinks).length} liens utiles...`
    );
    const validatedUsefulLinks = await this.validateLinksWithGemini(
      usefulLinks,
      tool,
      'useful'
    );
...
Write the article now in markdown format with H2 titles:`;
  ...
  private static async callGeminiWithFallback(prompt: string): Promise<string> {
    ...
    for (const modelName of this.GEMINI_MODELS) {
      try {
        console.log(
          `🔄 Tentative avec modèle: ${modelName}${modelName === 'gemini-2.5-pro' ? ' (PRIORITÉ 1)' : ''}`
        );
        ...
        console.log(
          `✅ Contenu généré avec succès par ${modelName} (${text.length} caractères)`
        );
        return text;
      } catch (error: any) {
        lastError = error;
...
DESCRIPTION: [max 160 chars with CTA]`;
  ...
  static async generateToolPricingModel(
    ...
    try {
      ...
      const crawledContent = crawledPages
        .slice(0, 5)
        .map(page => `${page.title}: ${page.content.substring(0, 1200)}...`)
        .join('\n\n');
...
Pricing Model:`;
      ...
      let cleanPricing = pricingResponse
    ...
    results.forEach((result, index) => {
      ...
      if (result.status === 'fulfilled' && result.value) {
        ...
        translations[fieldName] =
          `[ERREUR: ${result.status === 'rejected' ? result.reason?.message : 'Réponse vide'}]`;
        console.log(
          `❌ ${fieldName} échoué (${langCode.toUpperCase()}): ${result.status === 'rejected' ? result.reason?.message : 'Réponse vide'}`
        );
      }
    });

    console.log(
      `📊 Traduction ${langCode.toUpperCase()}: ${successfulFields}/7 champs réussis`
    );

    // 🚨 VALIDATION MINIMUM: Au moins 4/7 champs doivent réussir
    if (successfulFields < 4) {
      throw new Error(
        `Traduction ${langCode} insuffisante: seulement ${successfulFields}/7 champs réussis (minimum: 4)`
      );
    }
