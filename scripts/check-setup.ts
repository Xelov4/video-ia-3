  ...
  for (const placeholder of placeholders) {
    ...
    if (!fs.existsSync(fullPath)) {
      try {
        ...
        try {
          await execAsync(`convert -size ${width}x${height} plasma:blue-purple ${fullPath}`)
          console.log(`✅ Placeholder créé: ${placeholder}`)
        } catch (error) {
          // Fallback: Create an empty file if ImageMagick is not available
          fs.writeFileSync(fullPath, '')
          console.log(`⚠️ Placeholder vide créé (ImageMagick non disponible): ${placeholder}`)
        }
      } catch (error) {
        console.error(`❌ Erreur lors de la création du placeholder ${placeholder}:`, error)
      }
    } else {
  ...
  for (const lang of supportedLanguages) {
    ...
    if (!fs.existsSync(fullPath)) {
      try {
        ...
        try {
          await execAsync(`convert -size ${width}x${height} gradient:blue-purple -gravity Center -pointsize 40 -fill white -annotate 0 "Video-IA.net (${lang.toUpperCase()})" ${fullPath}`)
          console.log(`✅ OG image créée: ${ogImage}`)
        } catch (error) {
          // Fallback: Create an empty file if ImageMagick is not available
          fs.writeFileSync(fullPath, '')
          console.log(`⚠️ OG image vide créée (ImageMagick non disponible): ${ogImage}`)
        }
      } catch (error) {
...
async function main() {
  ...
  if (dbConnected) {
    ...
    console.log('📝 Vérifiez que PostgreSQL est démarré et que les identifiants dans .env.local sont corrects.')
  }
