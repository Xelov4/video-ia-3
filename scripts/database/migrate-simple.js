...
/**
 * Script de migration simplifié pour importer les données CSV
 */
...
async function migrateData() {
  console.log('🚀 Démarrage de la migration simplifiée...\n');
  ...
  try {
    ...
    await execAsync(`sudo -u postgres psql -d video_ia_net -c "${createTablesSQL}"`);
    ...
    for (let i = 0; i < dataLines.length; i += batchSize) {
      ...
      for (const line of batch) {
        try {
          ...
          const toolName = values[0]?.replace(/'/g, "''") || '';
          const category = values[1]?.replace(/'/g, "''") || '';
          const link = values[2]?.replace(/'/g, "''") || '';
          const overview = values[3]?.replace(/'/g, "''") || '';
          const description = values[4]?.replace(/'/g, "''") || '';
          const targetAudience = values[5]?.replace(/'/g, "''") || '';
          const keyFeatures = values[6]?.replace(/'/g, "''") || '';
          const useCases = values[7]?.replace(/'/g, "''") || '';
          const tags = values[8]?.replace(/'/g, "''") || '';
          const imageUrl = values[9]?.replace(/'/g, "''") || '';
          ...
          if (toolName && toolName.trim().length > 0) {
            ...
            const insertSQL = `
              INSERT INTO tools (
                ...
                '${toolName}', '${category}', '${link}', '${overview}', '${description}',
                '${targetAudience}', '${keyFeatures}', '${useCases}', '${tags}', '${imageUrl}', '${slug}'
              ) ON CONFLICT (slug) DO NOTHING;
            `;
            
            await execAsync(`sudo -u postgres psql -d video_ia_net -c "${insertSQL}"`);
            insertedCount++;
          }
        } catch (error) {
          errorCount++;
    ...
    await execAsync(`sudo -u postgres psql -d video_ia_net -c "
