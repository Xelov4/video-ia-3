...
async function testSommerAI() {
  ...
  try {
    ...
    if (result.socialLinks && Object.keys(result.socialLinks).length > 0) {
      console.log(
        `   ✅ ${Object.keys(result.socialLinks).length} réseaux sociaux détectés:`
      );
      ...
      console.log(
        `   💡 Les patterns recherchés : LinkedIn, Facebook, X/Twitter, GitHub, Discord, Instagram, TikTok`
      );
    }
    ...
    if (result.usefulLinks && Object.keys(result.usefulLinks).length > 0) {
      console.log(
        `   ✅ ${Object.keys(result.usefulLinks).length} liens utiles détectés:`
      );
    ...
    console.log(
      `   ${detailedReport.stepByStepResults.step3_socialExtraction.success ? '✅' : '❌'} Social Links (${detailedReport.stepByStepResults.step3_socialExtraction.data.linksFound} trouvés)`
    );
    console.log(
      `   ${detailedReport.stepByStepResults.step4_usefulLinks.success ? '✅' : '❌'} Useful Links (${detailedReport.stepByStepResults.step4_usefulLinks.data.linksFound} trouvés)`
    );
    console.log(
