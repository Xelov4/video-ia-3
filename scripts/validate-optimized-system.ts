...
async function validateOptimizedSystem() {
  ...
  try {
    ...
    const serviceExists = await fs
      .access('/root/video-ia.net/src/lib/services/toolContentUpdaterOptimized.ts')
      .then(() => true)
      .catch(() => false);
    ...
    if (serviceExists) {
      ...
      if (
        typeof ToolContentUpdaterServiceOptimized.updateToolContentWithTranslations ===
        'function'
      ) {
    ...
    const archiveExists = await fs
      .access('/root/video-ia.net/archives')
      .then(() => true)
      .catch(() => false);
    ...
    const oldServiceExists = await fs
      .access('/root/video-ia.net/src/lib/services/toolContentUpdater.ts')
      .then(() => true)
      .catch(() => false);
    ...
    if (testTool) {
      ...
      console.log(
        '   ⚠️  Test système complet non exécuté (utilisez test-multilingual-optimized.ts)'
      );
    } else {
    ...
    console.log(
      '   1. Utiliser: ToolContentUpdaterServiceOptimized.updateToolContentWithTranslations()'
    );
